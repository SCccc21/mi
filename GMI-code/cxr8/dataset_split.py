...
def load_cxr_private(base_path):
	datafile = '{}/Data_Entry_2017_v2020.csv'.format(base_path)

	private_train = os.path.join(base_path, 'train.txt')
	private_test = os.path.join(base_path, 'test.txt')
	f_private_train = open(private_train, "w")
	f_private_test = open(private_test, "w")
	# col_types = {'Image Index': str,
	#              'Finding Labels': float,
	#              'Follow-up #': float,
	#              'Patient ID': float,
	#              'Patient Age': int,
	#              'Patient Gender': int,
	...
	#              'dose': float}
	cnt_private = 0
	cxr = pd.read_csv(datafile)
	# import pdb; pdb.set_trace()
	i = 0
	while(cnt_private<10000):
		entry = cxr['Image Index'].iloc[i]
		# private
		if cxr['Finding Labels'].iloc[i] in ["Atelectasis", "Cardiomegaly",  "Effusion", "Infiltration", "Mass", "Nodule", "Pneumonia"]:
			...
			if cnt_private <= 2000:
				if cxr['Finding Labels'].iloc[i] == "Atelectasis":
					...
					f_private_test.write(entry + ' ' + str(1))
				elif cxr['Finding Labels'].iloc[i] == "Effusion":
					f_private_test.write(entry + ' ' + str(2))
				elif cxr['Finding Labels'].iloc[i] == "Infiltration":
					f_private_test.write(entry + ' ' + str(3))
				elif cxr['Finding Labels'].iloc[i] == "Mass":
					f_private_test.write(entry + ' ' + str(4))
				elif cxr['Finding Labels'].iloc[i] == "Nodule":
					f_private_test.write(entry + ' ' + str(5))
				elif cxr['Finding Labels'].iloc[i] == "Pneumonia":
			...
			# train
				if cxr['Finding Labels'].iloc[i] == "Atelectasis":
					...
					f_private_train.write(entry + ' ' + str(1))
				elif cxr['Finding Labels'].iloc[i] == "Effusion":
					f_private_train.write(entry + ' ' + str(2))
				elif cxr['Finding Labels'].iloc[i] == "Infiltration":
					f_private_train.write(entry + ' ' + str(3))
				elif cxr['Finding Labels'].iloc[i] == "Mass":
					f_private_train.write(entry + ' ' + str(4))
				elif cxr['Finding Labels'].iloc[i] == "Nodule":
					f_private_train.write(entry + ' ' + str(5))
				elif cxr['Finding Labels'].iloc[i] == "Pneumonia":
					f_private_train.write(entry + ' ' + str(6))
				f_private_train.write('\n')
...
def load_cxr_public(base_path):
	...
	while(cnt < 10000):
		entry = cxr['Image Index'].iloc[i]
		# public
		if cxr['Finding Labels'].iloc[i] in ["Pneumothorax", "Consolidation", "Edema", "Emphysema", "Fibrosis", "Pleural_Thickening", "Hernia", "No Finding"]:
			# print(i)
			# print(cnt)
			cnt += 1
			f_public.write(entry)
...
if __name__ == '__main__':
	base_path = '/home/sichen/data/cxr8'

	load_cxr_private(base_path)
	load_cxr_public(base_path)
