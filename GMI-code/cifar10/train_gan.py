...
save_img_dir = "./Attack/result/imgs_celeba_gan"
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1) % 10 == 0:
            ...
            fake_image = G(z)
            save_tensor_images(fake_image.detach(), os.path.join(save_img_dir, "result_image_{}_cifar.png".format(epoch)), nrow = 8)
        
        torch.save({'state_dict':G.state_dict()}, os.path.join(save_model_dir, "cifar_G.tar"))
        torch.save({'state_dict':DG.state_dict()}, os.path.join(save_model_dir, "cifar_D.tar"))
