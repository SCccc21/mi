...
class GBlock(nn.Module):
	def __init__(self,
				 ...
				 fmap_max=100,       # Maximum number of feature maps in any layer.
				 ):
...
class G_synthesis(nn.Module):
	def __init__(self,
				 ...
				 resolution=64,                    # Output resolution (1024 x 1024 by default).
				 fmap_base=8192,                     # Overall multiplier for the number of feature maps.
...
class StyleDiscriminator(nn.Module):
	def __init__(self,
		...
		self.nf = lambda stage: min(int(fmap_base / (2.0 ** (stage * fmap_decay))), fmap_max)
		# fromrgb: fixed mode
		self.fromrgb = nn.Conv2d(num_channels, 32, kernel_size=1)
		self.structure = structure
		...
		self.down22 = nn.Conv2d(100, 100, kernel_size=2, stride=2)
		self.down23 = nn.Conv2d(100, 100, kernel_size=2, stride=2)
		self.down24 = nn.Conv2d(100, 100, kernel_size=2, stride=2)
		...
		self.conv5 = nn.Conv2d(32, 64, kernel_size=3, padding=(1, 1))
		self.conv6 = nn.Conv2d(64, 100, kernel_size=3, padding=(1, 1))
		self.conv7 = nn.Conv2d(100, 100, kernel_size=3, padding=(1, 1))
		self.conv8 = nn.Conv2d(100, 100, kernel_size=3, padding=(1, 1))

		# calculate point:
		self.conv_last = nn.Conv2d(100, 100, kernel_size=3, padding=(1, 1))
		self.dense0 = nn.Linear(1600, 100)
	...
	def forward(self, input):
		if self.structure == 'fixed':
			...
			'''
			# 5. 64 x 64 -> 32 x 32
			res = self.resolution_log2 #res=6
